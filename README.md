...
- **大图预览关闭按钮让出窗口控制条热区（866583d）**：壳窗口控制条 `#dsh-desktop-win-controls` 恒 fixed top0/高40/z-index 2147483647，而官方 `ImageLightbox`（top20/right20）与 vision-router 便桥浮层（top16/right18）的关闭按钮中心都落在控制条热区内——点击被上层拦截，甚至可能误触最小化/关窗。dshvt CSS 注入两条 `!important` 规则把两类关闭按钮统一压到壳发布的安全区令牌之下：`top:calc(var(--dsh-titlebar-safe,44px) + 10px)`、`right:24px`（旧壳无令牌取 44 兜底）；选择器锚 role/aria 与 CSS module 类名后缀，防构建哈希漂移，双轨通用。纯 dshvt 注入（问题68 令牌消费侧），无需动壳或上游
